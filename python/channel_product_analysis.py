...
# 加入線上線下
df = df.merge(channels, left_on='FirstChannel', right_on='Channel', how='left')


df = df[['FirstTransactionYear','CustomerId','buy_count','ProductType','ProductType_road','ChannelType']]

# new_ord
new_ord = df.groupby(['FirstTransactionYear','ChannelType']).agg({'CustomerId': 'nunique'})\
    .rename(columns={'CustomerId':'new_ord'})

# rep_ord
rep_ord = df[df['buy_count']>=2]\
    .groupby(['FirstTransactionYear','ChannelType'])\
    .agg({'CustomerId': 'nunique'})\
    .rename(columns={'CustomerId':'rep_ord'})

#只買核心商品
only_core = df[(df['ProductType'].notnull())&(df['ProductType_road'].isnull())]\
    .groupby(['FirstTransactionYear','ChannelType']).agg({'CustomerId': 'nunique'})\
...
result = new_ord.merge(rep_ord,on=['FirstTransactionYear','ChannelType'],)\
    .merge(only_core,on=['FirstTransactionYear','ChannelType'])\
    .merge(only_road,on=['FirstTransactionYear','ChannelType']).\
    merge(both,on=['FirstTransactionYear','ChannelType']).\
    merge(others,on=['FirstTransactionYear','ChannelType'])
print(result)
